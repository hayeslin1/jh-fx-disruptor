 ...
 * <p>
 * 特性：
 * 1. 同 ccyPair 高频推送时，{@link #merge} 只保留时间戳较新的一条（与 FxXrInfMapper.xml 的
 *    orderGuard 语义一致：按 DT_CHANNEL_PUBLISH + TM_CHANNEL_PUBLISH 比较）；
...
public class FxRateLatestBuffer {
     ...
     * 合并一条新推送。
     * <p>
     * 时间戳保序：若 map 中已有同 key 的旧值，比较 DT+TM_CHANNEL_PUBLISH，只有新值不早于旧值才覆盖。
     * 这与数据库侧 {@code orderGuard} 的语义一致，防止"晚到的旧推送"污染报价。
     *
     * @param key      唯一业务键
    ...
    public void merge(LatestKey key, FxRateEvent incoming) {
        map.merge(key, incoming, (oldE, newE) -> {
            String oldTs = safe(oldE.getDtChannelPublish()) + safe(oldE.getTmChannelPublish());
            String newTs = safe(newE.getDtChannelPublish()) + safe(newE.getTmChannelPublish());
